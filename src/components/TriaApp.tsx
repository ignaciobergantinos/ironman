...
// selector para añadir un alimento extra (fuera del plan) desde el catálogo, o crear uno nuevo
...
// bloque interactivo de una comida: por defecto nada está comido; marca lo que comas,
// ajusta cantidades y añade extras. El plan (cantidades por defecto) es la plantilla.
function MealCard({ meal, log, date, byId, catalog, api }: {
  meal: Meal; log: MealLog | undefined; date: string; byId: Record<string, Food>; catalog: Food[]; api: UseStore;
}) {
...
// barra de progreso de proteína del día vs el objetivo del plan
function ProteinBar({ have, target }: { have: number; target: number }) {
  const pct = target > 0 ? Math.min(100, (have / target) * 100) : 0;
  const hit = target > 0 && have >= target;
  return (
    <div className="pbar">
      <div className="pbar-top">
        <span className="pbar-lab">Proteína</span>
        <span className="pbar-val mono"><b style={{ color: hit ? "var(--good)" : "var(--accent)" }}>{Math.round(have)}</b> / {Math.round(target)} g</span>
      </div>
      <div className="pbar-track"><div className="pbar-fill" style={{ width: pct + "%", background: hit ? "var(--good)" : "var(--accent)" }} /></div>
    </div>
  );
}
function FoodView({ api }: { api: UseStore }) {
  ...
  return (
    <>
      ...
      <div className="card food-total">
        <div className="card-lab"><span className="eyebrow">Total del día</span><span className="tag">marca lo que comas</span></div>
        <MacroCols m={total} head />
        <ProteinBar have={total.p} target={target.p} />
        <div className="food-vs mono">Objetivo del plan · {Math.round(target.kcal)} kcal · {Math.round(target.p)} P · {Math.round(target.c)} C · {Math.round(target.fat)} G</div>
      </div>
